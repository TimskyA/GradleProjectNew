...
public class id1774 {//частично дописан
    ...
    @Test
    public void step03() {Buttons.common("Вернуть значения по умолчанию");}
    ...
    public void step05() throws InterruptedException {
        ...
        assertTrue(Unzip.file("\\Users\\"+System.getProperty("user.name")+"\\Downloads\\CCReport.zip"));//.file вернёт false если не найдет в архиве хотя бы одного из файлов
    ...
    public void step06_07_08_09() throws IOException, InterruptedException {//пришлось объединить все шаги с 6го по 9й в один, что б не объявлять экземпляры классов XSSFWorkbook и PdfReader как поле
        try(FileInputStream xlsxStream=new FileInputStream("\\Users\\"+System.getProperty("user.name")+"\\Downloads\\CCReport\\"+Regex.nameLastCalculation+"_Export.xlsx")){
        XSSFWorkbook xslxReport = new XSSFWorkbook(xlsxStream);
        ...
        //8й шаг
        try(FileInputStream pdfStream = new FileInputStream("\\Users\\"+System.getProperty("user.name")+"\\Downloads\\CCReport\\CCReport1.pdf")) {
